    ...
    public class NewSpaceMappingProfile : Profile
    {
        ...
        public NewSpaceMappingProfile()
        {
            CreateMap<HtmlNodeCollection, NewSpaceExternalListItemModel>()//too vague and sometimes a better solution seems to be a custom mapping
                ...
                .ForMember(u => u.PricePerKg, opts => opts.MapFrom(v => string.Join("", v[8].InnerText.TakeWhile(z => char.IsDigit(z)))))
                .ForMember(u => u.IsFundingExist, opts => opts.MapFrom(v => v[9].InnerText.Contains("Yes") || v[9].InnerText.Any(z => char.IsDigit(z))))
